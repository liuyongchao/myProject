define(function(require) {
	...
	var SqliteUtil = require("$UI/NingboHaijinguangfuAPP/js/sqliteUtil");

	var _instance = null;

	var DemoDB = function() {
//		在调试场景下，这里可以选择用HTML5的Web SQL Database，在Chrome浏览器远程调试时可以直接看到数据库内容
		this.database = window.openDatabase("X5_NBHJ_DB", null, null, -1);
	...
	DemoDB.prototype.initData = function(onSuccess) {
		...
		// 检查表是否已存在
		var sqlCheckExists = "SELECT count(*) AS cnt FROM sqlite_master WHERE type = 'table' AND UPPER(name) in ('HJ_USER') ";
		SqliteUtil.executeSql(db, sqlCheckExists, null, function(res) {
			if (res.rows.item(0).cnt < 2) {
				// 如果不存在则获取初始化脚本
				require(["text!$UI/NingboHaijinguangfuAPP/js/demoDB.sql"], function(data) {
					executeSqlFile(data);
				});
			} else {
				doSuccess();
